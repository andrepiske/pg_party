...
ActiveRecord::Schema.define do
  ...
  create_range_partition_of \
    :bigint_timestamp_ranges,
    name: :bigint_timestamp_ranges_a,
    start_range: time_now - (12 * 3600),
    end_range: time_now + (26 * 3600)

  create_range_partition_of \
    :bigint_timestamp_ranges,
    name: :bigint_timestamp_ranges_b,
    start_range: time_now + (26 * 3600),
    end_range: time_now + (40 * 3600)

  # Remove primary key from partitions so it can be created in the main table
  execute "ALTER TABLE bigint_timestamp_ranges_a DROP CONSTRAINT bigint_timestamp_ranges_a_pkey"
  execute "ALTER TABLE bigint_timestamp_ranges_b DROP CONSTRAINT bigint_timestamp_ranges_b_pkey"
  execute "ALTER TABLE bigint_timestamp_ranges ADD PRIMARY KEY (id, created_at)"
